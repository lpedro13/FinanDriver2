...
const ExpenseDistribution = ({ dados }) => {
  const totalAllExpenses = dados.totalDespesasTrabalho + dados.totalDespesasPessoais;

  const getWidthPercentage = (value) => {
    if (totalAllExpenses === 0) return '0%';
    return `${(value / totalAllExpenses) * 100}%`;
  };
    ...
    <Card>
      ...
      <CardContent>
        <div className="space-y-4">
          <div>
            <Label className="text-sm text-muted-foreground">Despesas de Trabalho</Label>
          ...
          <div>
            <Label className="text-sm text-muted-foreground">Proporção de Despesas (Carro, Comb., Pessoais, Reserva)</Label>
          ...
          <div>
            <Label className="text-sm text-muted-foreground">Eficiência Financeira</Label>
            <div className="mt-2">
              <div className="flex justify-between items-center">
                <span>Lucro / Ganhos</span>
                <span className="font-medium">
                  {dados.totalGanhos > 0 
